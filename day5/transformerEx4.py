...
class MaskedSelfAttention(nn.Module):
    ...
    def forward(
        ...
        mask = torch.tril(
            torch.ones(
                sequence_length,    #111   100 
                sequence_length,    #111   110
                device=x.device     #111   111  원래 행렬인 torch.one을 tril을 사용하여 오른쪽 상부를 0으로 날릴 수 있음
            )
        )
        print('\nmask:')
        print(mask)
        ...
        masked_score = scaled_score.masked_fill( #왜 masked_fill을 쓰는지 replace가 아니라
            mask ==0,
            float('-inf') #음수 무한대
        )
        print('\nmasked score')
